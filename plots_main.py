...
gs = GridSpec(3, 2, width_ratios=[1, 1], height_ratios=[1, 3, 3], hspace=0.4, wspace=0.4)
...
ax1.set_xlabel(r'Time $t$', color='white')
ax1.set_ylabel(r'Consensus level $|m(t)|$', color='white')
ax1.set_ylim([-0.01, 1.01])
ax1.set_xlim([0, 10])
...
# Create an inset axis for the box plot N=50
ax3 = inset_axes(ax1, width="20%", height="100%", loc='center right', borderpad=-4.75)
ax3.set_facecolor('white')  # inset background transparent
...
for model_name in unique_models:
    ...
    if key in final_values:
        color = model_to_color[model_name]
        ax3.boxplot(
            np.abs(final_values[key]),
            positions=[i], widths=[0.75], patch_artist=True, 
            boxprops=dict(facecolor=color, alpha=0.3), 
            medianprops=dict(color=color, linewidth=7), 
            flierprops=dict(markerfacecolor=color)
        )
...
ax_inset = ax2.inset_axes([0.1, 0.6, 0.35, 0.35])
ax_inset.set_facecolor('white')  # inset background transparent

# Make tick labels white in the inset
...
# Axis labels in white
ax2.set_xlabel(r'Collective opinion $m$', color='white')
ax2.set_ylabel(r'Adoption probability $P(m)$', color='white')
ax2.set_ylim([-0.1, 1.3])
...
leg = ax_legend.legend(new_handles, labels, loc='upper center', fontsize=22, ncol=5, bbox_to_anchor=(0.47, 0.75))
...
ax_legend.set_facecolor('none')  # make the legend axis background transparent
...
lower_bound_models = ["GPT-4 Turbo", "Claude 3.5 Sonnet", "GPT-4"]
...
ax3.set_xlabel('Intelligence (MMLU Benchmark score)', fontsize=20)
...
for file in cw_files1:
    N = int(os.path.basename(file).split('_')[0][1:])
    data_cw = pd.read_csv(file, delim_whitespace=True, header=None)
    consensus_times = data_cw.iloc[:, -1]  # Last column
    mean_time = consensus_times.mean()
    std_time = consensus_times.std()
    curie_weiss_data1.append((N, mean_time, std_time))
...
plt.rcParams.update({'font.size': 20})
gs = GridSpec(2, 2, width_ratios=[1, 1], height_ratios=[15, 1], hspace=0.4, wspace=0.3)
ax2 = fig.add_subplot(gs[0, 0])
ax3 = fig.add_subplot(gs[:, 1])
...
for model_name, values in mean_std_data.items():
    ...
    label = filename_to_legend[model_name]
    ax2.errorbar(
        np.array(values['N'])[times_to_plot], mean_values, 
        yerr=error, label=label, capsize=5, 
        marker=color_marker_map[label][1], color=color_marker_map[label][0]
    )

# Adding Curie-Weiss model data
ax2.errorbar(
    curie_weiss_df1['N'], curie_weiss_df1['mean'], yerr=curie_weiss_df1['std'],
    label=r'Curie-Weiss model', capsize=5, marker='x', color='grey'
)

# Axis labels in white
ax2.set_xlabel(r'Group Size $N$', fontsize=20, color='white')
ax2.set_ylabel(r'Consensus Time $T_c$', fontsize=20, color='white')
...
# Legend
handles, labels = ax2.get_legend_handles_labels()
leg = fig.legend(handles, labels, loc='lower center', ncol=2, bbox_to_anchor=(0.29, 0.05))
...
for i in range(len(labels_dp)):
    if labels_dp[i] in lower_bound_models:
        color = color_marker_map[labels_dp[i]][0]
        marker = color_marker_map[labels_dp[i]][1]
        ax3.scatter(x[i], y[i], marker=marker, s=200, color=color)
        used_color = color
        
        log_yi = np.log10(y[i])
        arrowhead_log_y = log_yi + 0.2
        arrowhead_y = 10 ** arrowhead_log_y
        ...
        if labels_dp[i] in arrows:
            ax3.scatter(x[i], y[i], marker=arrows[labels_dp[i]], s=300, color='black')
            used_color = 'black'
        elif labels_dp[i] == "Humans":
            ax3.scatter(x[i], y[i], marker='H', s=300, color='pink')
            used_color = 'pink'
        else:
            color = color_marker_map[labels_dp[i]][0]
    ...
    xytext = offsets.get(labels_dp[i], (5, 5))
    ax3.annotate(
        labels_dp[i],
        (x[i], y[i]),
        textcoords="offset points",
...
ax3.set_xlabel('Language Understanding score (MMLU)', fontsize=20, color='white')
ax3.set_ylabel(r'Maximal group size $N_{c}$', fontsize=20, color='white')
ax3.set_yscale('log')
ax3.set_xlim([69, 94])
ax3.set_ylim([1, 2200])
ax3.grid(True)
...
plt.tight_layout(pad=0)
plt.savefig('plot3.png', dpi=300, bbox_inches='tight', transparent=False)  # <--- Transparent figure
plt.show()
